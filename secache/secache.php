...
class secache {
    ...
    var $data_base_pos = 262588; //40($idx_node_size)+20($header_padding)+24*16+16*16*16*16*4;   
    ...
    /**
     * 写入缓存内容
     * @param int $offset           文件写入其实位置
     * @param string $data          写入内容
     * @return boolean              写入结果
     */
    function _fwrite($offset, $data) {
    ...
    /**
     * 锁定文件，如果系统不支持flock方法，需要重载此方法,写入缓存内容是使用独占锁
     * @param type $is_block    发生锁定时是否阻塞
     * @return type
     */
    function lock($is_block) {
        //客户端连接中断，脚本继续执行
        ignore_user_abort(1);
    ...
    /**
     * 获取缓存文件最大大小，默认定义为15M，缓存文件最大1G，最小10M
     * @param string $str_size      缓存文件大小
     * @param int $default          默认文件大小
    ...
    function _parse_str_size($str_size, $default) {
        ...
        if (preg_match('/^([0-9]+)\s*([gmk]|)$/i', $str_size, $match)) {
            ...
            if ($size <= 0) {
                ...
                //文件内容最小 10485760 = 10 * 1024 * 1024 = 10M
                return 10485760;
            } elseif ($size > 1073741824) {
                //文件内容最大不得超过1G，1073741824 = 1 * 1024 * 1024 * 1024 = 1G 返回错误
    ...
    /**
     * 
     * @param boolean $truncate     是否清空文件内容
     * @return boolean
     */
    function _format($truncate = false) {
        //写入文件内容是锁定文件，如果锁定失败，返回错误信息
        if (!$this->lock(true)) {
        ...
        $this->_fwrite($this->header_padding, pack('V1a*', $this->max_size, $this->ver));
        ksort($this->_bsize_list);
        $ds_offset = $this->data_base_pos;
        $i = 0;
        foreach ($this->_bsize_list as $size => $count) {
            //将预分配的空间注册到free链表里
            $count *= min(3, floor($this->max_size / 10485760));  //10485760 = 10 * 1024 * 1024 = 10M
            $next_free_node = 0;
            for ($j = 0; $j < $count; $j++) {
                $this->_fwrite($ds_offset, pack('V', $next_free_node));
            ...
            $this->_fwrite(60 + $i * $this->schema_item_size, $code);
            $i++;
        }
        //将数据段地址写入文件中
        $this->_set_dcur_pos($ds_offset);

        $this->_fwrite($this->idx_base_pos, str_repeat("\0", 262144));
        $this->_fwrite($this->idx_seq_pos, pack('V', 1));
        $this->unlock();
        return true;
    }
    ...
    function _set_node_root($key, $value) {
        return $this->_fwrite(hexdec(substr($key, 0, 4)) * 4 + $this->idx_base_pos, pack('V', $value));
    }
    ...
    function _set_node($pos, $key, $value) {
        ...
        if (isset($this->_node_struct[$key])) {
            return $this->_fwrite($pos * $this->idx_node_size + $this->idx_node_base + $this->_node_struct[$key][0], pack($this->_node_struct[$key][1], $value));
        } else {
            return false;
        }
    }
    ...
    function _set_dcur_pos($pos) {
        return $this->_fwrite($this->dfile_cur_pos, pack('V', $pos));
    }
    ...
    function _free_dspace($size, $pos) {
        ...
        if ($free = $this->_get_schema($schema_id, 'free')) {
            $this->_fwrite($free, pack('V1', $pos));
        } else {
            $this->_set_schema($schema_id, 'free', $pos);
        }
        $this->_fwrite($pos, pack('V1', 0));
    }
    ...
    function _free_node($pos) {
        ...
        $this->_fwrite($pos * $this->idx_node_size + $this->idx_node_base, pack('V', $prev_free_node) . str_repeat("\0", $this->idx_node_size - 4));
        return $this->_fwrite($this->idx_free_pos, pack('V', $pos));
    }
    ...
    function _alloc_idx($data) {
        ...
        if ($list_pos) {
            ...
            list(, $prev_free_node) = unpack('V', fread($this->_rs, 4));
            $this->_fwrite($this->idx_free_pos, pack('V', $prev_free_node));
        } else {
            $this->_seek($this->idx_seq_pos);
            list(, $list_pos) = unpack('V', fread($this->_rs, 4));
            $this->_fwrite($this->idx_seq_pos, pack('V', $list_pos + 1));
        }
        return $this->_create_node($list_pos, $data);
    }

    function _create_node($pos, $data) {
        $this->_fwrite($pos * $this->idx_node_size + $this->idx_node_base
    ...
    function _set_schema($schema_id, $key, $value) {
        ...
        return $this->_fwrite(60 + $schema_id * $this->schema_item_size + $info[$key] * 4, pack('V', $value));
    }
